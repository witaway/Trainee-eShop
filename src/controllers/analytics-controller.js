...
class AnalyticsController {
    static async getProduct(req, res) {
        ...
        if (result.length === 0) {
            throw new NotFoundException(
                'There is no product like you need. Maybe there is no marks in month?',
            );
        }
        ...
        res.status(200).json(
            ResponseFormat.success(200, 'Analytics info is got successfully', {
                product: product,
    ...
    static async getUser(req, res) {
        ...
        if (result.length === 0) {
            throw new NotFoundException(
                'There is no user like you need. Maybe there is no marks in database?',
            );
        }
