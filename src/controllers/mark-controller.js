...
class MarkController {
    static async get(req, res) {
        ...
        res.status(200).json(
            ResponseFormat.success(200, 'Mark is got successfully', result),
        );
    }

    static async set(req, res) {
        const result = await MarkService.setMark(
            req.user,
            req.params.id,
            req.body.value,
        );
        res.status(200).json(
            ResponseFormat.success(200, 'Mark is set successfully', result),
        );
    }
    ...
    static async delete(req, res) {
        ...
        res.status(200).json(
            ResponseFormat.success(200, 'Mark is deleted successfully', {}),
        );
    }
